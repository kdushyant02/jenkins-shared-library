...
def call(String buildResult,String SLACK_CHANNEL) {
  if ( buildResult == "SUCCESS" ) {
    ...
    slackSend color: "warning", channel: "${params.SLACK_CHANNEL}", message: "*ABORTED:* Job ${env.JOB_NAME} build ${env.BUILD_NUMBER} by ${env.USER_ID}\n More info at: ${env.BUILD_URL}"
  }
  else if( buildResult == "UNSTABLE" ) { 
    slackSend color: "warning", channel: "${params.SLACK_CHANNEL}", message: "*UNSTABLE:* Job ${env.JOB_NAME} build ${env.BUILD_NUMBER} by ${env.USER_ID}\n More info at: ${env.BUILD_URL}"
  }
  else 
  {
